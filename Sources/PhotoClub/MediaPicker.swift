...
import SwiftUI
import OSLog
fileprivate let logger: Logger = Logger(subsystem: "skip.kit", category: "SkipKit") // adb logcat '*:S' 'skip.kit.SkipKit:V'
...
extension View {
    ...
    // SKIP @nobridge
    @ViewBuilder public func withMediaPicker(type: MediaPickerType, isPresented: Binding<Bool>, selectedImageURL: Binding<URL?>, detents: Set<PresentationDetent> = [.large]) -> some View {
        switch type {
        case .library:
            #if !SKIP
            #if os(iOS)
